...
def toMetadata(log, gmetadata, ExHentai_Status, Chinese_Status,sqlitUrl):  # {{{
    ...
    mi.identifiers = {'lanraragi': '%s' % (str(gid))}
...
class Ehentai(Source):
    ...
    touched_fields = frozenset(['title', 'authors', 'tags', 'rating', 'publisher', 'identifier:lanraragi'])
    ...
    options = (
        ...
        Option('Query_Interval', 'number', 0, _('Query Interval'),
               _('设置查询间隔时间（秒），防止频繁查询导致IP被封禁')),
        ...
        Option('Use_Lanraragi', 'bool', False, _('Use Lanraragi'),
               _('If enabled, will try to fetch metadata from Lanraragi first')),
        Option('Lanraragi_URL', 'string', 'http://192.168.31.136:2951', _('Lanraragi URL'),
               _('The URL of your Lanraragi server'))
    )
    ...
    def __init__(self, *args, **kwargs):  # {{{  
        ...
        self.config_tags()
        self.config_proxy()
        self.config_query_interval()
    # }}}
    
    def config_query_interval(self):
        self.query_interval = self.prefs['Query_Interval']
        return
    ...
    def get_book_url(self, identifiers):  # {{{
        db = identifiers.get('lanraragi', None)
        if db is None:
            return None
        try:
            gid = db.split('_')[0]  # 我们只需要gid部分
            lanraragi_url = self.prefs['Lanraragi_URL']
            url = f"{lanraragi_url}/reader?id={gid}"
            return ('lanraragi', db, url)
    ...
    def get_html_content(self,br,query,is_exhentai,log, result_queue, abort, title, authors, identifiers, timeout):
        try:
            # 添加查询间隔
            if hasattr(self, 'query_interval') and self.query_interval > 0:
                import time
                log.info('等待查询间隔: %s 秒' % self.query_interval)
    ...
    def get_metadata_from_lanraragi(self, log, title, authors=None):
        if not self.prefs['Use_Lanraragi']:
            return None
            
        # Process author name
        author = ''
        if authors and authors[0] not in ['Unknown', '未知']:
            author = authors[0]
        ...
        try:
            # 使用calibre的browser对象发送请求
            br = self.browser
            lanraragi_url = self.prefs['Lanraragi_URL']
            url = f"{lanraragi_url}/api/search?filter={search_query}"
            
            # 发送API请求
            raw = br.open_novisit(url, timeout=30).read()
            data = json.loads(raw)
            ...
            if data.get('data') and len(data['data']) > 0:
                ...
                gmetadata = {
                    'title': title,
                    'title_jpn': result.get('title', ''),
                    'tags': result.get('tags', '').split(',') if result.get('tags') else [],
                    'rating': 5.0,  # Default rating
                    'category': 'Doujinshi',  # Default category
                    'gid': result.get('arcid', ''),
                    'token': 'lanraragi',
                    'thumb': result.get('thumbnail', '')
                }
                
                return toMetadata(log, gmetadata, False, self.Chinese_Status, self.sqlitUrl)
            else:
                # 如果lanraragi没有返回数据，返回None以继续使用e-hentai搜索
                log.info('从lanraragi没有找到数据，将继续使用e-hentai搜索')
                return None
        except Exception as e:
            log.error(f"从lanraragi获取metadata失败: {str(e)}")
            return None

    def identify(self, log, result_queue, abort, title=None, authors=None, identifiers={}, timeout=30):
        # 首先尝试从lanraragi获取metadata
        if self.prefs['Use_Lanraragi']:
            lanraragi_metadata = self.get_metadata_from_lanraragi(log, title, authors)
            if lanraragi_metadata:
                result_queue.put(lanraragi_metadata)
                return
                
        # 如果lanraragi没有结果，继续使用原有的e-hentai搜索逻辑
